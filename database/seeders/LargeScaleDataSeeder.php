...
class LargeScaleDataSeeder extends Seeder
{
    ...
    {
        $this->command->info('Starting large-scale data generation...');
        $this->command->info('This will create 100M+ records. Press Ctrl+C to abort.');
    ...
    {
        $this->command->info('Generating base data (users, salaries, departments, designations)...');
        ...
        $this->command->info("Creating 100,000 employees...");
        ...
        $this->command->info("Created {$totalEmployees} employees with salaries.");
    }

    private function generateAttendanceData(): void
    {
        $this->command->info('Generating attendance data (60 months)...');
        ...
        $this->command->info("Using chunk size: " . number_format($this->chunkSize));
        ...
        for ($month = $months; $month >= 1; $month--) {
            ...
            for ($day = 1; $day <= $daysPerMonth; $day++) {
                $attendanceDate = $monthStart->copy()->addDays($day - 1);
    ...
    private function generateLeaveData(): void
    {
        ...
        for ($i = 0; $i < $totalLeaves; $i++) {
            ...
            if (count($batch) >= $batchSize) {
                ...
                if ($i % 50000 === 0) {
                    $this->command->info("  Leaves: " . number_format($i) . "/" . number_format($totalLeaves));
                }
            }
        }
    ...
    {
        $this->command->info('Generating leave balances...');
        ...
        foreach ($years as $year) {
            ...
            $this->command->info("  Year {$year} balances created");
        }
    ...
    private function getRandomLeaveType(): string
    {
        $types = ['casual', 'sick', 'annual', 'unpaid', 'maternity', 'paternity'];
